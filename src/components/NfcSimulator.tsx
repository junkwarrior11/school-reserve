...
export default function NfcSimulator({ teachers, resources, history, onRefresh, onSelectResourceForInspection }: NfcSimulatorProps) {
  ...
  // Helper to run actual tap API for classroom
  const proceedClassroomUsage = async (tagId: string) => {
    setIsScanning(true);
    setScanResult(null);
    try {
      ...
      setIsScanning(false);
      const registeredRes = resources.find(r => r.nfcTagId === tagId || r.qrCodeId === tagId);
  ...
  const handleSimulateScan = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // INTERCEPT: If the resource is a classroom, prompt choices
        if (registeredRes && registeredRes.category === 'classroom') {
          setIsScanning(false);
          setPendingClassroom(registeredRes);
          setPendingTagId(idToScan);
          setShowClassroomChoice(true);
          return;
        }

        // Otherwise (it's equipment or a teacher tag), proceed as usual
        const response = await fetch('/api/nfc/tap', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            ...
            >
              <div className="p-6">
                ...
                <div className="bg-slate-50 border border-slate-150 p-4 rounded-xl my-5">
                  ...
                  <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
                    <User className="w-3.5 h-3.5" /> 操作教員: {activeTeacher?.name || '不明'} 先生
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    onClick={() => {
                      setShowClassroomChoice(false);
                      proceedClassroomUsage(pendingTagId);
                    }}
